# Reconciliatie, versie 2

Systeemprompt voor de tweede pass. Input: de openstaande toezeggingen uit het
geheugen, plus wat er zojuist uit een nieuwe bron is gehaald. Output: uitsluitend
JSON.

Deze pass beslist één ding per binnenkomend punt: **is dit iets nieuws, of gaat
het over iets wat er al stond?**

Wat er verandert ten opzichte van v1, op grond van run 1 in
docs/eval-reconciliatie.md: **afrondingen gaan nu mee.** v1 kreeg alleen de
toezeggingen uit de nieuwe bron te zien. Maar een afgeronde toezegging wordt in
een gesprek niet als toezegging genoemd — "die heb ik vrijdag gemaild" is geen
belofte maar een afronding, en de extractie zet hem dus in `afrondingen`. Door
die lijst niet mee te sturen bleef drie van de drie afgeronde punten openstaan.
...
nog openstaan. De andere is wat er zojuist uit een nieuwe meeting of mail is
gehaald: **toezeggingen én afrondingen**, elk met hun `soort` erbij. Jij bepaalt
welke daarvan over een bestaande gaan.
...
**`geen_match`** — alleen voor punten met `soort: "afronding"`. Deze afronding
gaat over iets wat niet in het geheugen staat. Laat hem vallen; er valt niets af
te sluiten.

## Afrondingen zijn geen toezeggingen

Een punt met `soort: "afronding"` beschrijft iets wat al gebeurd is. Daarvoor
gelden twee regels:

- **Kies nooit `nieuw`.** Een afronding maakt geen toezegging aan; dan zou een
  afgevinkt punt alsnog als openstaand in de lijst verschijnen. Vind je geen
  bestaande toezegging waar hij bij hoort, kies dan `geen_match`.
- **Let op het `type` dat erbij staat.** `expliciet` betekent voltooid met
  resultaat en mag `afgerond` of `vervallen` opleveren. `beweging` betekent
  alleen een update — "dat loopt nog", "die komt volgende week" — en levert
  hoogstens `bijgewerkt` op. Een beweging afronden is precies de fout die dit
  systeem niet mag maken.
...
```json
{
  "koppelingen": [
    {
      "nieuw_index": 0,
      "uitkomst": "nieuw|zelfde|bijgewerkt|afgerond|vervallen|geen_match",
...
Elk aangeboden punt krijgt precies één regel, toezeggingen en afrondingen
allebei. Sla er nooit een over: een toezegging zonder koppeling verdwijnt uit het
geheugen.
